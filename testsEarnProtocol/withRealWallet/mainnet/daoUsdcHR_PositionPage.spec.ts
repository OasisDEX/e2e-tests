...
test.describe('With real wallet - DAO Mainnet USDC Higher Risk position page - Deposit', async () => {
	...
	test('It should show Deposit balances and Deposit amounts - DAO Mainnet USDC Higher Risk position page', async ({
		app,
	}) => {
		// === USDC ===
	...
	test('It should deposit USDC & WETH - (until rejecting "Deposit" tx) - DAO Mainnet USDC Higher Risk position page', async ({
		app,
		metamask,
	}) => {
...
test.describe('With real wallet - DAO Mainnet USDC Higher Risk position page - Withdraw', async () => {
	test.beforeEach(async ({ app, metamask }, testInfo) => {
		testInfo.setTimeout(testInfo.timeout + 110_000);
	...
	test('It should show maximum USDC balance amount to be withdrawn in $ - DAO Mainnet USDC Higher Risk position page', async ({
		app,
	}) => {
	...
	test('It should withdraw to USDC - (until rejecting "Withdraw" tx) - DAO Mainnet USDC Higher Risk position page', async ({
		app,
		metamask,
	}) => {
		await withdraw({
			...
			withdrawAmount: '0.4',
			estimatedEarnings: {
				thirtyDaysAmount: '0.1[0-9]{3}',
				sixMonthsAmount: '0.1[0-9]{3}',
				oneYearAmount: '0.1[0-9]{3}',
				threeYearsAmount: '0.1[0-9]{3}',
			},
			previewInfo: {
				transactionFee: '[0-9]{1,2}.[0-9]{2}',
			},
...
test.describe('With real wallet - DAO Mainnet USDC Higher Risk position page - Switch', async () => {
	test.beforeEach(async ({ app, metamask }, testInfo) => {
		testInfo.setTimeout(testInfo.timeout + 110_000);
	...
	test('It should display info about original and target vaults - Switch DAO Mainnet USDC Higher Risk position', async ({
		app,
	}) => {
	...
	test('It should switch DAO Mainnet USDC Higher Risk position', async ({ app, metamask }) => {
		test.setTimeout(veryLongTestTimeout);
