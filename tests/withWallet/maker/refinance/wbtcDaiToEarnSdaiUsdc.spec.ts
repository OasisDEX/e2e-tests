...
test.describe('Maker Borrow - Wallet connected', async () => {
	...
	test('It should refinance a Maker Borrow position (WBTC/DAI) to Spark EARN (SDIA/USDC) @refinance', async () => {
		...
		await app.position.refinance.selectReason('Switch to an Earn position');
		await app.position.refinance.productList.byPairPool('SDAI/USDC Yield Loop').open();
		...
		await app.position.manage.shouldBeVisible('Manage your Spark Earn Position');
