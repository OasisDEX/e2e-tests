...
test.describe('Maker Borrow - Wallet connected', async () => {
	...
	test('It should refinance a Maker Borrow position (WBTC/DAI) to Spark Borrow (sDAI/ETH) @regression', async () => {
		test.info().annotations.push({
			type: 'Test case',
			description: '11788, 11790',
		});
		...
		await app.position.refinance.selectReason('Change direction of my position');
		await app.position.refinance.productList.byPairPool('SDAI/ETH').open();
