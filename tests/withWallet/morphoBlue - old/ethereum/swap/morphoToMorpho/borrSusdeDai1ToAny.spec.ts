...
test.describe('Morpho Blue Borrow - Swap to Morpho', async () => {
	...
	test('It should open a Morpho Blue Borrow position - SUSDE/SUSDE-DAI-1', async () => {
		...
		await app.page.goto('/ethereum/morphoblue/borrow/SUSDE-DAI-1#setup');
	...
	(
		...
		test(`It should swap a Morpho Borrow position (SUSDE/DAI-1) to Morpho Multiply (${targetPool.colToken}/${targetPool.debtToken})`, async () => {
			test.info().annotations.push({
				type: 'Test case',
				description: 'xxx',
			});
