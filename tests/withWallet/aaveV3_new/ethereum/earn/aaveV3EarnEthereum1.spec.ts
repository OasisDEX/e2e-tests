...
import { manageDebtOrCollateral, openPositionNEW } from 'tests/sharedTestSteps/positionManagement';
...
test.describe('Aave V3 Earn - Ethereum - Wallet connected', async () => {
	...
	test('It should open an Aave V3 Earn Ethereum position @regression', async () => {
		...
		await test.step('Test setup', async () => {
			({ context } = await metamaskSetUp({ network }));
			let page = await context.newPage();
			app = new App(page);

			({ walletAddress } = await setup({
				app,
				network,
				withoutFork: true,
				extraFeaturesFlags: 'AaveV3LambdaSuppressValidation:true AaveV3SuppressValidation:true',
			}));
		});

		await app.position.openPage('/ethereum/aave/v3/earn/wsteth-eth#simulate');

		({ forkId } = await openPositionNEW({
			walletAddress,
			network,
			addTokenBalance: { token: 'WSTETH', balance: '50' },
			app,
