...
test.describe('Aave v3 Multiply Base', async () => {
	test('It should allow to simulate an Aave V3 Base Multiply position before opening it - No wallet connected', async ({
		app,
	}) => {
		...
		await app.position.overview.shouldHaveLoanToValueAfterPill('[0-9]{2}.[0-9]{2}%');
	...
	test('It should allow to simulate an Aave V3 Base Multiply position before opening it - Adjust risk - Up and Down  - No wallet connected', async ({
		app,
	}) => {
		test.info().annotations.push({
			type: 'Test case',
