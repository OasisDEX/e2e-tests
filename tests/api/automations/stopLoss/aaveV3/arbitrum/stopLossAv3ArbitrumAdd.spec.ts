...
const validPayloads = validPayloadsAaveV3Arbitrum.stopLoss.closeToDebt;

test.describe('API tests - Stop-Loss - Add - Aave V3 - Arbitrum', async () => {
	...
	test('Add automation - Close to debt - Valid payload data', async ({ request }) => {
		...
		expect(respJSON).toMatchObject({
			...
			transaction: {
				...
				to: '0xF0464Ef55705e5B5cb3b865d92BE5341fe85fBB8',
			},
		});
	});
	...
	test('Add automation - Trigger already exists', async ({ request }) => {
		const response = await request.post(stopLossEndpoint, {
			data: { ...validPayloadsAaveV3Arbitrum.stopLoss.updateCloseToCollateral, action: 'add' },
		});
